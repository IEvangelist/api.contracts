...
public sealed class ApiSchemaGenerator : IIncrementalGenerator
{
        ...
        AssemblyConfig config)
    {
        ...
        // Emit the raw JSON wrapped in #if false so MSBuild targets can
        // extract it to a standalone .json file during build.
        context.AddSource(
            $"{assemblyName}.api-schema.g.cs",
            SourceText.From(
                "// <auto-generated/>\n" +
                "// API Contract data file — extracted by ApiContracts.Generator.targets\n" +
                "#if false\n" +
                schemaJson + "\n" +
                "#endif\n",
                Encoding.UTF8));
        ...
        context.AddSource(
            $"{assemblyName}.api-schema.json.g.cs",
            SourceText.From(
